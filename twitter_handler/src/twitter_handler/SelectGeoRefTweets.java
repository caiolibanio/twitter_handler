...
public class SelectGeoRefTweets {
	...
	private static void filteringRows() throws SQLException {
		...
		while (!lastLine) {
			try {
				...
				rs = st.executeQuery("select * from tweets_london_raw_original where tid between " + step1 + " and "
						+ step2 + " order by tid");
				step1 = step1 + 1000;
				step2 = step2 + 1000;
				while (rs.next()) {
	...
	private static boolean checkTweet(Tweet tweet) {
		try {
			...
			// System.out.println("Esta linha nao possui coordenadas
			// geograficas!");
		}
		return false;
	}
	...
	public static void insert(Tweet tweet) throws SQLException {
		...
		String sql = "INSERT INTO geo_tweets" + "(" + "tid" + ", " + "json" + ", " + "user_id" + ", " + "longitude"
				+ ", " + "latitude" + ", " + "message" + ", " + "date" + ") " + "VALUES (" + "'" + tweet.getTid() + "'"
				+ ", " + "$token$" + tweet.getJson() + "$token$" + ", " + "'" + tweet.getUser_id() + "'" + ", " + "'"
				+ tweet.getLongitude() + "'" + ", " + "'" + tweet.getLatitude() + "'" + ", " + "$token$"
				+ tweet.getMessage() + "$token$" + ", " + "'" + tweet.getDate() + "'" + ");";
